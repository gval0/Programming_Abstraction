...
// helper function
void helper(Node* root, Map<ext_char, string>& map, string soFar){
	if(root == NULL){
		// vicekvot yvelam ertad
	}else {
		if(root->character != NOT_A_CHAR) map[root->character] = soFar;
		helper(root->zero, map, soFar + '0');
		helper(root->one, map, soFar + '1');
	}
}
/* Function: encodeFile
...
void encodeFile(istream& infile, Node* encodingTree, obstream& outfile) {
	...
	while(infile.get(character)){
		...
		for(int i = 0; i < temp.size(); i++){
			if(temp[i] == '0') outfile.writeBit(0);
...
void decodeFile(ibstream& infile, Node* encodingTree, ostream& file) {
	int num;
	Node* cur = encodingTree;
	while(true){
		if(cur->character == PSEUDO_EOF) break;
		if(cur->character != NOT_A_CHAR){
			file.put((char)cur->character);
			cur = encodingTree;
		}
		num = infile.readBit();
		if(num == 0) cur = cur->zero;
		else cur = cur->one;
	}
}
